...
function onp_op_admin_premium_notices( $notices ) {
    ...
    $wasClosed = isset( $closed['onp-op-offer-to-purchase'] );

    $lastCloase  = isset( $closed['onp-op-offer-to-purchase'] ) 
        ? $closed['onp-op-offer-to-purchase']['time'] 
        : 0;

    $neverShow = isset( $closed['onp-op-offer-to-purchase'] ) 
        ? intval( $closed['onp-op-offer-to-purchase']['never'] )
        : false;
    
    // shows in 7 days
    if ( ( !$neverShow && (time() - $lastCloase > 60*60*7) ) || $forceToShowNotices ) {
